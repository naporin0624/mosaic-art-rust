...
use iced::{Application, Command, Element, Font, Length, Theme};
...
fn t_with_args(
    id: &str,
    args: std::collections::HashMap<String, fluent_bundle::FluentValue>,
) -> String {
...
fn auto_calculate_max_usage_per_image_with_force(
    ...
    if settings.max_usage_per_image > 0 && !force_recalculate {
        #[cfg(test)]
        println!(
            "auto_calculate_max_usage_per_image: keeping existing value {}",
    ...
    match settings.total_tiles {
        Some(total_tiles) if total_tiles > 0 && settings.max_materials > 0 => {
            ...
            #[cfg(test)]
            println!(
                "auto_calculate_max_usage_per_image: calculated {} / {} = {}, result={}",
...
impl Application for MosaicApp {
    ...
    fn new(_flags: Self::Flags) -> (Self, Command<Self::Message>) {
        ...
        let initial_language = if requested
            .first()
            .map(|l| l.language == "ja")
            .unwrap_or(false)
        {
        ...
        loader()
            .load_languages(&Localisations, &[&initial_language.langid()])
        ...
        match message {
            ...
            Message::TotalTilesChanged(value) => {
                ...
                if self.auto_calculate_max_usage {
                    ...
                    self.settings.max_usage_per_image =
                        auto_calculate_max_usage_per_image_with_force(&self.settings, true);
            ...
            Message::MaxMaterialsChanged(value) => {
                ...
                if let Ok(max) = value.parse::<usize>() {
                    ...
                    if self.auto_calculate_max_usage {
                        ...
                        // Force recalculation since we're in auto-calculation mode
                        let new_usage =
                            auto_calculate_max_usage_per_image_with_force(&temp_settings, true);
            ...
            Message::MaxUsagePerImageChanged(value) => {
                ...
                if let Ok(max) = value.parse::<usize>() {
                    if max == 0 {
                        ...
                        self.settings.max_usage_per_image =
                            auto_calculate_max_usage_per_image(&self.settings);
                        self.max_usage_per_image_input =
            ...
            Message::GenerateMosaic => {
                ...
                if self.target_path.is_empty() {
                    self.log_messages
                        .push("❌ Error: No target image selected".to_string());
                    return Command::none();
                }
                if self.material_path.is_empty() {
                    self.log_messages
                        .push("❌ Error: No material directory selected".to_string());
                    return Command::none();
                }
                if self.output_path.is_empty() {
                    self.log_messages
                        .push("❌ Error: No output path specified".to_string());
                    return Command::none();
                }

                // Create progress channel
                let (progress_sender, progress_receiver) =
                    mpsc::unbounded_channel::<(f32, String)>();
                ...
                self.log_messages
                    .push("🚀 Starting mosaic generation...".to_string());
                self.log_messages
                    .push(format!("📁 Target: {}", self.target_path));
                self.log_messages
                    .push(format!("📁 Materials: {}", self.material_path));
                self.log_messages
                    .push(format!("📁 Output: {}", self.output_path));
                self.log_messages.push(format!(
                    "🔧 Grid: {}x{} ({} tiles)",
                    self.settings.grid_w,
                    self.settings.grid_h,
                    self.settings.grid_w * self.settings.grid_h
                ));
                self.log_messages
                    .push(format!("⚙️ Max materials: {}", self.settings.max_materials));
                self.log_messages.push(format!(
                    "🎨 Color adjustment: {:.1}",
                    self.settings.color_adjustment
                ));
                self.log_messages.push(format!(
                    "🔧 Optimization: {}",
                ...
                match result {
                    Ok(output_path) => {
                        ...
                        if let Some(start_time) = self.start_time {
                            ...
                            self.log_messages
                                .push("✅ Mosaic generation completed".to_string());
                        }
                        self.log_messages
                            .push(format!("💾 Saved to: {output_path}"));
                    }
                    Err(error) => {
            ...
            Message::LanguageChanged(language) => {
                if language != self.current_language {
                    ...
                    loader()
                        .load_languages(&Localisations, &[&language.langid()])
                        .expect("Failed to load language catalog");
                }
            }
        }
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        ...
        let title = create_text(t("app-title")).size(36);

        // Language selection section
        let language_section = column![
            create_text(t("language-label")).size(16),
            pick_list(
                &UiLanguage::ALL[..],
        ...
        let files_section = column![
            create_text(t("file-selection-title")).size(24),
            column![
                create_text(t("target-image-label")).size(14),
                row![
            ...
            .spacing(4),
            column![
                create_text(t("material-directory-label")).size(14),
                row![
            ...
            .spacing(4),
            column![
                create_text(t("output-path-label")).size(14),
                row![
        ...
        let grid_section =
            column![
                create_text(t("grid-settings-title")).size(24),
                create_text(t("grid-settings-description")).size(14).style(
                    iced::theme::Text::Color(iced::Color::from_rgb(0.5, 0.5, 0.5))
                ),
                checkbox(t("auto-calculate-label"), self.settings.auto_calculate)
                    .on_toggle(Message::AutoCalculateToggled)
                    .spacing(8),
                create_text(t("auto-calculate-description")).size(12).style(
                    iced::theme::Text::Color(iced::Color::from_rgb(0.5, 0.5, 0.5))
                ),
                if self.settings.auto_calculate {
                    column![
                        column![
                            row![
                                create_text(t("total-tiles-label")).size(14),
                            ...
                            create_text(t("total-tiles-description")).size(12).style(
                                iced::theme::Text::Color(iced::Color::from_rgb(0.5, 0.5, 0.5))
                            )
                        ]
                        .spacing(4),
                ...
                row![
                    column![
                        create_text(t("grid-width-label")).size(14),
                        text_input(&t("grid-width-placeholder"), &self.grid_w_input)
                            .on_input(Message::GridWidthChanged)
                            .padding(8)
                            .width(Length::Fixed(100.0)),
                        create_text(t("grid-width-description")).size(12).style(
                            iced::theme::Text::Color(iced::Color::from_rgb(0.5, 0.5, 0.5))
                        )
                    ]
                    .spacing(4),
                    column![
                        create_text(t("grid-height-label")).size(14),
                        text_input(&t("grid-height-placeholder"), &self.grid_h_input)
                            .on_input(Message::GridHeightChanged)
                            .padding(8)
                            .width(Length::Fixed(100.0)),
                        create_text(t("grid-height-description")).size(12).style(
                            iced::theme::Text::Color(iced::Color::from_rgb(0.5, 0.5, 0.5))
                        )
                    ]
                    .spacing(4)
        ...
        let arrow = if self.advanced_settings_expanded {
            "▼ "
        } else {
            "► "
        };
        ...
        let advanced_section =
            if self.advanced_settings_expanded {
                column![
                    advanced_header,
                    container(
                        column![
                            create_text(t("configuration-title")).size(16).style(
                                iced::theme::Text::Color(iced::Color::from_rgb(0.6, 0.6, 0.6))
                            ),
                            create_text(t("configuration-description")).size(12).style(
                                iced::theme::Text::Color(iced::Color::from_rgb(0.5, 0.5, 0.5))
                            ),
                            column![
                                row![
                                    ...
                                    text_input(
                                        &t("color-adjustment-placeholder"),
                                        &self.color_adjustment_input
                                    )
                            ...
                            column![
                                row![
                                    create_text(t("max-usage-per-image-label"))
                                        ...
                                        .width(Length::Fixed(250.0)),
                                    text_input(
                                        &t("max-usage-per-image-placeholder"),
                                        &self.max_usage_per_image_input
                                    )
                            ...
                            column![
                                row![
                                    create_text(t("adjacency-penalty-weight-label"))
                                        ...
                                        .width(Length::Fixed(250.0)),
                                    text_input(
                                        &t("adjacency-penalty-weight-placeholder"),
                                        &self.adjacency_penalty_weight_input
                                    )
                            ...
                            column![
                                row![
                                    create_text(t("similarity-db-path-label"))
                                        ...
                                        .width(Length::Fixed(250.0)),
                                    text_input(
                                        &t("similarity-db-path-placeholder"),
                                        &self.similarity_db_path_input
                                    )
                            ...
                            column![
                                checkbox(
                                    t("rebuild-similarity-db-label"),
                                    self.settings.rebuild_similarity_db
                                )
                            ...
                            .spacing(4),
                            create_text(t("optimization-title")).size(16).style(
                                iced::theme::Text::Color(iced::Color::from_rgb(0.6, 0.6, 0.6))
                            ),
                            create_text(t("optimization-description")).size(12).style(
                                iced::theme::Text::Color(iced::Color::from_rgb(0.5, 0.5, 0.5))
                            ),
                            checkbox(
                                t("enable-optimization-label"),
                                self.settings.enable_optimization
                            )
                                ...
                                column![column![
                                    row![
                                        create_text(t("optimization-iterations-label"))
                                            ...
                                            .width(Length::Fixed(250.0)),
                                        text_input(
                                            &t("optimization-iterations-placeholder"),
                                            &self.optimization_iterations_input
                                        )
                            ...
                            create_text(t("debugging-title")).size(16).style(
                                iced::theme::Text::Color(iced::Color::from_rgb(0.6, 0.6, 0.6))
                            ),
                            create_text(t("debugging-description")).size(12).style(
                                iced::theme::Text::Color(iced::Color::from_rgb(0.5, 0.5, 0.5))
                            ),
                            column![
        ...
        let status_section = match &self.processing_state {
            ...
            ProcessingState::Loading => column![
                create_text(t("progress-initializing")).size(18),
                progress_bar(0.0..=1.0, 0.0).height(Length::Fixed(8.0))
            ]
            .spacing(8),
            ProcessingState::Processing { progress, step } => column![
                create_text(step.clone()).size(16),
                progress_bar(0.0..=1.0, *progress).height(Length::Fixed(12.0)),
                create_text(format!("{:.1}%", progress * 100.0)).size(14)
            ]
            .spacing(8),
            ProcessingState::Completed => column![create_text(t("status-completed")).size(18)],
            ProcessingState::Error(error) => {
                column![create_text(format!("❌ {}: {}", t("error-processing"), error)).size(16)]
            }
        }
        .padding(12);
        ...
        let generate_button = if is_processing {
            button(create_text(generate_button_text)).padding([12, 24])
        } else {
        ...
        let log_section = if !self.log_messages.is_empty() {
            let log_content = column(
                self.log_messages
                    ...
                    .rev()
                    .take(50)
                    .map(|msg| text(msg).size(13).into())
                    .collect::<Vec<Element<Message>>>(),
            )
            .spacing(2);

            column![
                create_text(t("generation-log-title")).size(24),
                container(scrollable(log_content).height(Length::Fixed(300.0)))
                    .padding(12)
                    .width(Length::Fill)
            ]
            .spacing(8)
        ...
        let main_content = column![
            container(title)
                ...
                .center_x()
                .width(Length::Fill),
            container(language_section).width(Length::Fill),
            container(files_section).width(Length::Fill),
            container(grid_section).width(Length::Fill),
            container(advanced_section).width(Length::Fill),
...
impl InternalMosaicGenerator {
    ...
    fn find_best_tile_for_position(
        ...
        eprintln!(
            "⚠️ PRIMARY SELECTION FAILED for position ({}, {}), trying fallback...",
            position.x, position.y
        );
        if let Some(tile) = self.fallback_tile_selection(target_lab, position) {
            eprintln!(
                "✅ FALLBACK SELECTION SUCCESS for position ({}, {})",
                position.x, position.y
            );
            return Some(tile);
        }
        ...
        eprintln!(
            "⚠️ FALLBACK SELECTION FAILED for position ({}, {}), trying final fallback...",
            position.x, position.y
        );
        if let Some(tile) = self.final_fallback_selection(target_lab, position) {
            eprintln!(
                "✅ FINAL FALLBACK SUCCESS for position ({}, {})",
                position.x, position.y
            );
            return Some(tile);
        }
        ...
        eprintln!(
            "❌ CRITICAL: ALL FALLBACK METHODS FAILED for position ({}, {})",
            position.x, position.y
        );
        None
    }
    ...
    fn fallback_tile_selection(
        ...
        eprintln!(
            "🔄 FALLBACK: Resetting usage tracker for position ({}, {})",
            position.x, position.y
        );
        ...
        for candidate in candidates {
            ...
            if self.can_place_at_position(&tile.path, position) {
                eprintln!(
                    "🔄 FALLBACK SUCCESS: Found tile {} after resetting usage tracker",
...
fn generate_mosaic_internal(
    ...
    debug_log(&format!(
        "Loading target image from: {}",
        target_path.display()
    ));
    let target_img =
        image::open(&target_path).map_err(|e| format!("Failed to load target image: {e}"))?;

    send_progress(
        0.1,
        format!(
            "📸 Loaded target image: {}x{}",
            target_img.width(),
            target_img.height()
        ),
    );
    ...
    send_progress(
        0.15,
        format!(
            "📁 Loading material images from: {}",
            material_path.display()
        ),
    );
    debug_log("Scanning directory for image files (png, jpg, jpeg)");
    ...
    send_progress(
        0.2,
        format!("🎨 Found {} material images", material_files.len()),
    );
    debug_log(&format!(
        "Material files: {:?}",
        material_files
    ...
    debug_log(&format!(
        "Similarity database path: {}",
        similarity_db_path.display()
    ));
    let mut similarity_db = if similarity_db_path.exists() {
        debug_log("Loading existing similarity database");
        SimilarityDatabase::load_from_file(&similarity_db_path).unwrap_or_else(|e| {
            debug_log(&format!(
                "Failed to load similarity database: {e}, creating new"
            ));
    ...
    debug_log(&format!(
        "Similarity database built with {} tiles",
        tiles.len()
    ));
    ...
    debug_log(&format!(
        "Creating output image: {output_width}x{output_height}"
    ));
    ...
    for row in 0..settings.grid_h {
        ...
        for col in 0..settings.grid_w {
            ...
            if cell_index % (total_cells / 100).max(1) == 0 || verbose {
                ...
                if verbose {
                    debug_log(&format!("Grid progress: {percentage:.1}%"));
                } else {
                    send_progress(
                        overall_progress,
                        format!("⚙️ Processing grid: {percentage:.1}%"),
                    );
                }
            }
            ...
            if verbose {
                debug_log(&format!(
                    "Cell ({}, {}): target Lab color = L={:.1} a={:.1} b={:.1}",
                    col + 1,
                    row + 1,
                    target_lab.l,
                    target_lab.a,
            ...
            if let Some(tile) = generator.find_best_tile_for_position(&target_lab, position) {
                ...
                let tile_placed = match image::open(&tile.path) {
                    Ok(tile_img) => {
                        ...
                        match (
                            ...
                            (Ok(src_image), mut dst_image) => {
                                ...
                                match resizer.resize(
                                    ...
                                    Ok(_) => {
                                        ...
                                        for (dy, row_pixels) in adjusted_pixels
                                            .chunks_exact(tile_width as usize * 3)
                                            .enumerate()
                                        {
                                            for (dx, pixel) in
                                                row_pixels.chunks_exact(3).enumerate()
                                            {
                                                let out_x = x + dx as u32;
                                                let out_y = y + dy as u32;

                                                if out_x < output_img.width()
                                                    && out_y < output_img.height()
                                                {
                            ...
                            _ => {
                                log_message(&format!(
                                    "⚠️ Failed to create resize images for tile {}",
                                    tile.path.display()
                                ));
                                false
                            }
                ...
                if !tile_placed {
                    log_message(&format!(
                        "⚠️ Using target region as fallback for cell ({}, {})",
                        col + 1,
                        row + 1
                    ));
                    ...
                    let target_resized =
                        if target_rgb.width() != tile_width || target_rgb.height() != tile_height {
                    ...
                    for (dy, row_pixels) in target_resized
                        .chunks_exact(tile_width as usize * 3)
                        .enumerate()
                    {
                ...
                log_message(&format!(
                    "❌ CRITICAL: No tile found for position ({}, {}) - using target region",
                    col + 1,
                    row + 1
                ));
                ...
                let target_rgb = target_region.to_rgb8();
                let target_resized = if target_rgb.width() != tile_width
                    || target_rgb.height() != tile_height
                {
                    // Resize the target region to match tile dimensions
                    let target_raw = target_rgb.into_raw();
                    match (
                        ...
                        FirImage::new(tile_width, tile_height, fast_image_resize::PixelType::U8x3),
                    ) {
                        (Ok(src_image), mut dst_image) => {
                            let resize_options = ResizeOptions::new().resize_alg(
                                fast_image_resize::ResizeAlg::Convolution(
                    ...
                    .enumerate()
                {
                    for (dx, pixel) in row_pixels.chunks_exact(3).enumerate() {
                        ...
                        if out_x < output_img.width() && out_y < output_img.height() {
                            output_img.put_pixel(out_x, out_y, Rgb([pixel[0], pixel[1], pixel[2]]));
                        }
                    }
                }
            }
    ...
    if settings.enable_optimization && settings.adjacency_penalty_weight > 0.0 {
        ...
        debug_log(&format!(
            "Optimization settings: max_iterations={}, adjacency_penalty_weight={:.2}",
        ...
        debug_log(&format!(
            "Optimization result: initial_cost={:.2}, final_cost={:.2}, iterations={}",
        ...
        send_progress(
            0.96,
            "🎨 Rebuilding mosaic with optimized placement...".to_string(),
        );
    ...
    send_progress(
        1.0,
        format!("✅ Mosaic saved to: {}", output_path.display()),
    );
    debug_log(&format!(
...
fn apply_color_adjustment(pixels: &[u8], adjustment: (f32, f32, f32), strength: f32) -> Vec<u8> {
    pixels
        .chunks_exact(3)
        .flat_map(|pixel| {
...
mod tests {
    ...
    fn test_generate_mosaic_internal_with_verbose_logging() {
        ...
        assert!(
            ...
            "Verbose mosaic generation should succeed"
        );
        ...
        assert!(
            result_non_verbose.is_ok(),
            "Non-verbose mosaic generation should succeed"
        );
        assert!(output_path.exists(), "Output file should be created");
    }
    ...
    fn test_mosaic_app_initial_verbose_state() {
        ...
        assert_eq!(
            app.settings.verbose_logging, false,
    ...
    fn test_mosaic_app_update_adjacency_penalty_weight() {
        ...
        app.update(Message::AdjacencyPenaltyWeightChanged(
            "invalid".to_string(),
        ));
    ...
    fn test_mosaic_app_update_optimization_iterations() {
        ...
        app.update(Message::OptimizationIterationsChanged(
            "invalid".to_string(),
        ));
    ...
    fn test_internal_mosaic_generator_creation() {
        ...
        let generator = InternalMosaicGenerator::new(tiles, 2, 2, 3, similarity_db, 0.3);
    ...
    fn test_internal_mosaic_generator_empty_tiles() {
        let tiles = Vec::new();
        let similarity_db = SimilarityDatabase::new();
        let mut generator = InternalMosaicGenerator::new(tiles, 2, 2, 3, similarity_db, 0.3);
        ...
        assert!(
            result.is_none(),
            "Should return None when no tiles are available"
        );
    }
    ...
    fn test_can_place_at_position() {
        ...
        let similarity_db = SimilarityDatabase::new();
        let mut generator = InternalMosaicGenerator::new(tiles, 3, 3, 3, similarity_db, 0.3);
    ...
    fn test_fallback_tile_selection_with_adjacency_constraints() {
        ...
        assert!(
            result2.is_some(),
            "Second placement should succeed with fallback"
        );
    }
    ...
    fn test_final_fallback_selection() {
        ...
        assert!(
            result.is_some(),
            "Final fallback should always succeed when tiles exist"
        );

        // Verify the tile was placed
        assert!(
            generator.placed_tiles[0][0].is_some(),
            "Tile should be placed in grid"
        );
    }
    ...
    fn test_mosaic_app_update_similarity_db_path() {
        ...
        app.update(Message::SimilarityDbPathChanged(
            "custom_db.json".to_string(),
        ));
    ...
    fn test_comprehensive_fallback_scenario() {
        ...
        for y in 0..2 {
            for x in 0..2 {
                ...
                assert!(
                    result.is_some(),
                    "All positions should be filled even with restrictive constraints"
                );
            }
        }
        ...
        for y in 0..2 {
            for x in 0..2 {
                assert!(
                    generator.placed_tiles[y][x].is_some(),
    ...
    fn test_find_best_tile_primary_with_detailed_logging() {
        ...
        assert!(
            result.is_some(),
            "Primary selection should succeed with available tiles"
        );
    ...
    fn test_step_by_step_max_usage_calculation() {
        ...
        assert_eq!(
            app.max_usage_per_image_input,
            expected_after_change.to_string()
        );
    }
    ...
    fn test_max_usage_calculation_with_different_material_counts() {
        ...
        for (materials, expected) in test_cases {
            ...
            assert_eq!(
                app.settings.max_usage_per_image, expected,
                "Failed for materials={}, expected={}",
                materials, expected
            );
